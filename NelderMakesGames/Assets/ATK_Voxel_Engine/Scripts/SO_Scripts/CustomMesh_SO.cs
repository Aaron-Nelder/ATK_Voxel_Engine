...
[CreateAssetMenu(fileName = "Custom_Mesh", menuName = DebugHelper.MENU_NAME + "/Custom_Mesh")]
public class CustomMesh_SO : ScriptableObject
{
    ...
    public Mesh SaveMesh()
    {
        ...
        mesh.SetIndices(Indicies, meshTopology, 0);
    ...
    public Plane[] GetVisiblePlanes(ChunkPosition chunkPos, Vector3Int worldPos)
    {
        List<Plane> visiblePlanes = new List<Plane>();

        foreach (var plane in Planes)
        {
            if (!WorldHelper.IsOccupied(chunkPos, worldPos + plane.Normal))
                visiblePlanes.Add(plane);
        }

        return visiblePlanes.ToArray();
    }
