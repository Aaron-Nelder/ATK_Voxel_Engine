...
namespace ATKVoxelEngine
{
    public static class EngineUtilities
    {
        ...
        public static bool VoxelCast(Ray ray, out VoxelCastHit selectedVoxel, float stepLength = 0.05f, float range = 5f)
        {
            ...
            for (float i = 0; i < range; i += stepLength)
            {
                ...
                if (WorldPosToChunk(wPos, out Chunk chunk, out int3 lPos, out uint id))
                {
        ...
        public static void DestroyVoxel(int3 vWorldPos)
        {
            ...
            chunk.RenderManager.RemoveVoxelVertices(vLocalPos);
        ...
        public static bool IsChunkVisible(Bounds chunkBounds)
        {
    ...
    public struct VoxelCastHit : IEquatable<VoxelCastHit>
    {
        ...
        public int3 VoxelWorldPos => Selector.SelectedVoxel.LocalPosition + Selector.SelectedVoxel.Chunk.WorldPosition;
        public int3 NormalWorldPos => Selector.SelectedVoxel.LocalPosition + Selector.SelectedVoxel.Chunk.WorldPosition + Selector.SelectedVoxel.Normal;

        public VoxelCastHit(Chunk chunk, int3 localPosition, uint id, int3 normal)
        {
            Chunk = chunk;
